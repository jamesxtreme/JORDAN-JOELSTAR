...
smd(
  ...
  async (message, input) => {
    try {
      const text = input.trim();
      if (!text) {
        return await message.send("*_Ask Miles anything. Eg facts about the University of Benin_*");
      }
      ...
      return await message.send(`𝕄𝕀𝕃𝔼𝕊 𝔹𝔸ℝ𝔻 𝔸𝕀: ${result}`, { quoted: message });
    } catch (error) {
      await message.error(error + "\n\nCommand: bard", error, "*Failed to use Miles.*");
    }
  }
);
smd({
      ...
      if (!_0xd9b857.includes("x")) {
        return _0x165087.send(
          "*You did not add x in number.*\n*Ex: " +
            prefix +
            "advt 2349054xx,Your_Message_here*  \n " +
            Config.caption
        );
      }
